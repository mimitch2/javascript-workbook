...
function Board(begin, end) {
  ...
  this.viewGrid = () => {  // prints out the board
    ...
    for (let row = 0; row < 8; row++) {// we begin with our row number in our array
  ...
  this.isMoveLegal = () => {
    ...
    if (this.grid[this.board.begin[0]][this.board.begin[1]] === turn && //only can move own checker AND
      this.grid[this.board.end[0]][this.board.end[1]] === null) { //only can move to empty spot
    ...
    if (turn === black) {
      if (this.board.end[0] === this.board.begin[0] + 2) { //if trying to move +2 rows black
        if (this.board.end[1] === this.board.begin[1] + 2) { //and +2 columns it's a right jump FIXME checking this twice
          if (this.grid[this.board.end[0] - 1][this.board.end[1] - 1] !== turn &&
            this.grid[this.board.end[0] - 1][this.board.end[1] - 1] !== null) { //check that offset piece is not black or null
            return true
          }
      ...
      if (this.board.end[0] === this.board.begin[0] - 2) { //if trying to move -2 rows red
        if (this.board.end[1] === this.board.begin[1] + 2) { //and +2 columns it's a right jump
          if (this.grid[this.board.end[0] + 1][this.board.end[1] - 1] !== turn &&
            this.grid[this.board.end[0] + 1][this.board.end[1] - 1] !== null) { //check that offset piece is not red or null
            return true
          }
  ...
  this.moveIt = () => { //FIXME need to get this working INSIDE moveChecker
    ...
    this.grid[this.board.begin[0]].splice([this.board.begin[1]], 1, null)
    this.grid[this.board.end[0]].splice([this.board.end[1]], 1, turn)
    if (turn === red) {
      turn = black;
    } else {
      turn = red;
...
function Game() {
  ...
  this.board = new Board([null, null], [null, null]); //makes new instance of Board class FIXME Maybe this should be move
